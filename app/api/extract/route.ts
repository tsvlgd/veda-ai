  ...
  try {
    ...
    if (!questionPaper || !answerSheet) {
      return NextResponse.json(
        { success: false, error: 'Both question paper and answer sheet are required.' },
        { status: 400 },
      )
    }
    ...
    for (const modelName of geminiModels) {
      ...
      try {
        ...
        if (is503) {
          console.log('[Gemini] 503 detected, trying next model after 2s...')
        ...
        const completion = await groq.chat.completions.create(
          {
            ...
            messages: [
              ...
              {
                role: 'user',
                content: `DOCUMENT 1 — QUESTION PAPER:\n${qpText}\n\nDOCUMENT 2 — STUDENT ANSWER SHEET:\n${asText}`,
              },
            ],
            temperature: 0.1,
          },
    ...
    if (!apiSuccess) {
      return NextResponse.json(
        {
          success: false,
          error: geminiError instanceof Error ? geminiError.message : 'All API calls failed.',
        },
        { status: 502 },
      )
    }
    ...
    if (!jsonMatch) {
      return NextResponse.json(
        { success: false, error: 'AI did not return valid JSON. Please retry.' },
        { status: 422 },
      )
    }
